...
import ca.mcgill.ecse439.pds.controller.InvalidInputException;
...
//		bgRenderer.setBackground(Color.decode("#FF6961"));
	...
	{		
		...
		try {
			String[] ins=((String)tableModel.getValueAt(i, 2)).replaceAll("<html>", "").replaceAll("</html>", "").split(", ");
			List<Ingredient> inlist=new ArrayList<>();
			for(String s:ins){
				Ingredient ingredient=Ingredient.getWithName(s);
				inlist.add(ingredient);
			}
			cont.updateMenuPizza(pizzas.get(i), (String)tableModel.getValueAt(i, 0),(double)tableModel.getValueAt(i,1),Integer.parseInt((String) tableModel.getValueAt(i, 3)),inlist.toArray(new Ingredient[0]));
